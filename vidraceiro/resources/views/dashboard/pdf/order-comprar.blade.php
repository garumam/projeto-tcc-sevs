...
@foreach($budgets as $budget)
    ...
    @forelse($budget->products()->get() as $product)
        <h4 style="text-align: center;">Produto - {{$product->mproduct()->first()->nome}} |
            unidades: {{$product->qtd}}</h4>
        <h4>Vidros</h4>
        ...
        @empty
            <p style="margin-left: 20px;">Nenhum vidro neste produto!</p>
        @endforelse
        <h4>Alumínios</h4>
        @forelse($product->aluminums as $aluminum)
            <p style="margin-left: 20px;">{{'* '.$aluminum->perfil.' | qtd: '.$aluminum->qtd.' | medida(unidade): '.$aluminum->medida.' | medida(total): '.($aluminum->medida * $aluminum->qtd * $product->qtd).' | Qtd de peças de alumínio(6m) que serão utilizadas: '}}
                ...
                @endif</p>
        @empty
            <p style="margin-left: 20px;">Nenhum alumínio neste produto!</p>
        @endforelse
        <h4>Componentes</h4>
        ...
        @empty
            <p style="margin-left: 20px;">Nenhum componente neste produto!</p>
        @endforelse
        <div class="line" style="margin-top: 20px;"></div>
    @empty
        <p>Nenhum produto cadastrado neste orçamento!</p>
    @endforelse
    ...
    @forelse($reservasNoEstoque as $reserva)
        ...
        @endif
    @empty
        <p style="margin-left: 20px;">Nada reservado no estoque para este orçamento!</p>
    @endforelse
